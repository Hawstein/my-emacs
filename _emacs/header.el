...
;; Last Modified By: Karsten Thygesen
;; Last Modified On: Wed Jun  1 15:15:43 1994
;; Update Count    : 157
;; Status          : OK
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
;; HISTORY 		
;; 13-Jun-1990  (Last Mod: Wed Jun 13 13:55:21 1990 #152)  Lars Bo Nielsen
;;    Added the header-mode-line, and changed the function slightly,
;;    not to put in a lot of comment-chars
;; 5-Jan-1990  (Last Mod: Thu Jan  4 16:17:31 1990 #144)  Lars Bo Nielsen
;;    Changed the make-revision 
;; 4-Jan-1990		Lars Bo Nielsen	
;;    Last Modified: Thu Jan  4 10:48:16 1990 #138 (Lars Bo Nielsen)
;;    Changed the layout of the header.
;; 23-Nov-1989		Lars P. Fischer	
;;    Last Modified: Thu Nov 23 12:12:48 1989 #134 (Lars P. Fischer)
;;    Changed SCCS setup. You have to be carefull with `@' when using WEB.
;; 21-Nov-1989		Lars P. Fischer	
;;    Last Modified: Tue Nov 21 14:05:07 1989 #131 (Lars P. Fischer)
;;    Changed to suit personal preferences, to work with c-support,
;;    and removed command-line-option-stuff. Also fixed update-file-header.
...
(defvar make-header-hooks '(header-mode-line
			    header-blank
			    header-file-name
			    header-blank
			    header-copyright
...
(defun header-creation-date ()
  ...
  (insert header-prefix-string "Created On      : "
	  (current-time-string) "\n"))
...
(defun header-rcs ()
  ...
  (insert header-prefix-string
	  ...
	  header-prefix-string
	  "$Log: header.el,v $
	  "Revision 1.1  2001/02/01 20:15:57  lasse
	  "nye filer
	  "\n"))

(defun header-sccs ()
  "Inserts a line to record sccs information."
  (insert header-prefix-string "SCCS Status     : %M% %I%\t%G%\n"))
...
(defun make-revision ()
  ...
  (let ((header-prefix-string (header-prefix-string))
	...
	(while (and (< (point) header-max)
		    (looking-at
		     (concat "[ \t]*\\("
			     ...
			     "\\)")))
	  (forward-line 1))
	(insert "\n" (if logical-comment-start logical-comment-start "")
		"HISTORY ")
	...
	(progn
	  (insert "  (Last Mod: " (buffer-substring (match-beginning 1)
						      (match-end 1)))
	  (if (save-excursion
